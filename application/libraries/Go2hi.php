...
// Author      : M. Nazir Arifin. @rif_first@yahoo.com
// -------------------------------------------------------------------
*/
...
class Go2hi {
	/**
	 * Menghasilkan string tanggal terformat berdasarkan $format, $ctype, dan $lang
	 *
	 * Public
	 * Optional @param str $format. String format tanggal dengan bentuk spt format date PHP
	 * Optional @param int $ctype. Jenis kalender, 0 = Gregorian, 1 = Hijri, -- lihat konstanta GO2HI
	 * Optional @param int $timestamp. Unix timestamp, jika kosong berarti saat ini
	 * Optional @param int $lang. Bahasa yang dihasilkan, 0 = Inggris, 1 = Indonesia
	 * @return string. String tanggal terformat
	 */
	...
	public static function date($format = '', $ctype = 0, $timestamp = 0, $lang = 0) {
		...
		self::$time 	= ( ! empty($timestamp) ? intval($timestamp) : time());
...
'\M\u\h','\S\a\f','\R\'\A\w','\R\'\A\k','\J\'\A\w','\J\'\A\k','\R\a\j','\S\y\a','\R\a\m','\S\a\w','\D\'\Q\d','\D\'\H\j');
	...
	protected static function hijri_num_days() {
		/*
		//hari ini
		$dist = (self::$dateH <= 15 ? (15-self::$dateH)*24*60*60 : 0);
	...
	protected static function replacef($char, $format) {
		...
		switch($char) {
			...
			case 'W': // (Angka minggu dalam satu tahun)
				if(self::isHijri()) {
					/*___________________________BAGAIMANA CARANYA??? ____________________*/
					return $format;
