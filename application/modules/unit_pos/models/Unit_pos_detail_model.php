...
class Unit_pos_detail_model extends CI_Model {
    ...
    {
        if($majors_id == "all"){
            $data = $this->db->query("SELECT `bebas_pay`.`bebas_pay_id`, `bebas_pay_bill`, `bebas_pay_bill`, `bebas_pay_number`, `bebas_pay_desc`, `bebas_pay_input_date`, `bebas_pay_last_update`, `bebas_pay`.`bebas_bebas_id`, `bebas_bill`, `student_student_id`, `student_nis`, `student`.`class_class_id`, `class_name`, `student_full_name`, `student_name_of_mother`, `student_parent_phone`, `payment_payment_id`, `payment_type`, `period_start`, `period_end`, `payment`.`pos_pos_id`, `pos_name`, `user_user_id`, `user_full_name`, `account_id`, `account_code` FROM `bebas_pay` LEFT JOIN `bebas` ON `bebas`.`bebas_id` = `bebas_pay`.`bebas_bebas_id` LEFT JOIN `student` ON `student`.`student_id` = `bebas`.`student_student_id` LEFT JOIN `payment` ON `payment`.`payment_id` = `bebas`.`payment_payment_id` LEFT JOIN `period` ON `period`.`period_id` = `payment`.`period_period_id` LEFT JOIN `pos` ON `pos`.`pos_id` = `payment`.`pos_pos_id` LEFT JOIN `class` ON `class`.`class_id` = `student`.`class_class_id` LEFT JOIN `users` ON `users`.`user_id` = `bebas_pay`.`user_user_id` LEFT JOIN `account` ON `account`.`account_id` = `pos`.`account_account_id` LEFT JOIN `majors` ON `majors`.`majors_id` = `account`.`account_majors_id` WHERE `payment`.`period_period_id` = '$period_id' AND account.account_code = '$paramFree' AND `bebas_pay_input_date` >= '$ds' AND `bebas_pay_input_date` <= '$de' AND majors.majors_status = '1' AND `bebas_pay`.`bebas_pay_account_id` = '$kasFree' ORDER BY `bebas_pay_last_update` ASC")->result_array();    
        } else {
            $data = $this->db->query("SELECT `bebas_pay`.`bebas_pay_id`, `bebas_pay_bill`, `bebas_pay_bill`, `bebas_pay_number`, `bebas_pay_desc`, `bebas_pay_input_date`, `bebas_pay_last_update`, `bebas_pay`.`bebas_bebas_id`, `bebas_bill`, `student_student_id`, `student_nis`, `student`.`class_class_id`, `class_name`, `student_full_name`, `student_name_of_mother`, `student_parent_phone`, `payment_payment_id`, `payment_type`, `period_start`, `period_end`, `payment`.`pos_pos_id`, `pos_name`, `user_user_id`, `user_full_name`, `account_id`, `account_code` FROM `bebas_pay` LEFT JOIN `bebas` ON `bebas`.`bebas_id` = `bebas_pay`.`bebas_bebas_id` LEFT JOIN `student` ON `student`.`student_id` = `bebas`.`student_student_id` LEFT JOIN `payment` ON `payment`.`payment_id` = `bebas`.`payment_payment_id` LEFT JOIN `period` ON `period`.`period_id` = `payment`.`period_period_id` LEFT JOIN `pos` ON `pos`.`pos_id` = `payment`.`pos_pos_id` LEFT JOIN `class` ON `class`.`class_id` = `student`.`class_class_id` LEFT JOIN `users` ON `users`.`user_id` = `bebas_pay`.`user_user_id` LEFT JOIN `account` ON `account`.`account_id` = `pos`.`account_account_id` LEFT JOIN `majors` ON `majors`.`majors_id` = `account`.`account_majors_id` WHERE `payment`.`period_period_id` = '$period_id' AND account.account_code = '$paramFree' AND `bebas_pay_input_date` >= '$ds' AND `bebas_pay_input_date` <= '$de' AND majors.majors_id = '$majors_id' AND majors.majors_status = '1' AND `bebas_pay`.`bebas_pay_account_id` = '$kasFree' ORDER BY `bebas_pay_last_update` ASC")->result_array();
        }
