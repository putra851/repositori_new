      ...
      <table class="table table-bordered" border=1 style="border-collapse:collapse">
        ...
        <tbody>
          ...
          foreach($rekap_absensi as $row):
            ...
            $ijin = $this->db->query("select id from data_absensi where id_pegawai='$idp' and (tanggal between '$tgl_awal' and '$tgl_akhir') and (jenis_absen = 'SAKIT' and 'IJIN' and 'LAIN-LAIN')");
            $ijin=$ijin->num_rows();

            $terlambat = $this->db->query("select id from data_absensi where id_pegawai='$idp' and (tanggal between '$tgl_awal' and '$tgl_akhir') and jenis_absen='DATANG' and time >= '07:00:00'");
