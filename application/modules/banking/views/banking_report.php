				...
				<div class="box box-success">
					<div class="box-header">
						...
						<div class="row">
							...
							<div class="col-md-2">  
								<div class="form-group">
									...
									<select class="form-control" name="m" id="student_id" >
										...
										<?php if(isset($q['c'])){?>
										<option <?php echo (isset($q['m']) AND $q['m'] == '0') ? 'selected' : '' ?> value="0">Semua Siswa</option>
							...
							<?php if ($q AND !empty($banking)) { ?>
							<a class="btn btn-success" target="_blank" href="<?php echo site_url('manage/banking/banking_excel' . '/?' . http_build_query($q)) ?>"><i class="fa fa-file-excel-o" ></i> Excel</a>
							<?php } ?>
						    </div>
					        </div>
					</div>
						...
						<table id="dtable" class="table table-responsive table-bordered" style="white-space: nowrap;">
							...
							?>
								<tr>
									...
									<td><a href="<?php echo site_url('manage/banking/printBook' . '/?' . 'n='.$q['p'].'& r='.$row['student_nis']) ?>" target="_blank" class="btn btn-sm btn-danger"><i class="fa fa-print"></i> Cetak Buku Tabungan</a></td>
								</tr>
							<?php endforeach ?>
                            </tbody>
						</table>
