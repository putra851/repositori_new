...
class Rekap_presensi_android_set extends CI_Controller {
  ...
  public function get_hari_month(){
        $p_tahun=date('Y');
        $p_bulan=date('m');
        
        $kalender=CAL_GREGORIAN;
        $hari=cal_days_in_month($kalender,$p_bulan,$p_tahun);
        return $hari;
    }
  ...
  public function index() {
    ...
    $max_hari=$this->get_hari_month();
    $periode_awal="$periode-01";
    $periode_akhir="$periode-$max_hari";
    ...
    $res = $this->db->query("select tanggal, min(jam_datang) jam_datang, max(jam_pulang) jam_pulang,catatan_absen,jenis_absen
        ...
        where tanggal between '$periode_awal' and '$periode_akhir'
        group by tanggal
        order by tanggal");
