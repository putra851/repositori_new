...
class Jurnal_model extends CI_Model {
    ...
    function get_Bulan($ds, $de, $majors_id ,$paramBulan, $kasBulan)
    {
        if($majors_id == "all"){
            $data = $this->db->query("SELECT DISTINCT `bulan`.`bulan_id`, `bulan_bill`, `bulan_date_pay`, `bulan_number_pay`,
                                    ...
                                    WHERE account.account_code = '$paramBulan' 
    ...
    function get_Free($ds, $de, $majors_id ,$paramFree, $kasFree)
    {
        if($majors_id == "all"){
            $data = $this->db->query("SELECT `bebas_pay`.`bebas_pay_id`, `bebas_pay_bill`, `bebas_pay_bill`, `bebas_pay_number`,
                                    ...
                                    WHERE account.account_code = '$paramFree' AND `bebas_pay_input_date` >= '$ds' 
            ...
            $data = $this->db->query("SELECT `bebas_pay`.`bebas_pay_id`, `bebas_pay_bill`, `bebas_pay_bill`, `bebas_pay_number`,
                                    ...
                                    LEFT JOIN `majors` ON `majors`.`majors_id` = `account`.`account_majors_id` 
                                    JOIN `kas` ON `bebas_pay`.`bebas_pay_noref` = `kas`.`kas_noref`
                                    WHERE account.account_code = '$paramFree' 
    ...
    function get_Debit($ds, $de, $majors_id ,$paramDebit, $kasDebit)
    {
        if($majors_id == "all"){
	...
	function get_Kredit($ds, $de, $majors_id ,$paramKredit, $kasKredit)
    {
        if($majors_id == "all"){
