...
class Users_model extends CI_Model {
    ...
    function get($params = array()) {
        ...
        if (isset($params['role'])) {
            $this->db->where('user_roles.role_id !=', $params['role']);
        }
        ...
        if($this->session->userdata('umajorsid') != '0'){
            $this->db->where('user_majors_id', $this->session->userdata('umajorsid'));
        }
        
        if($this->session->userdata('uroleid') != '3'){
            $this->db->where('user_role_role_id !=', 3);
        }
        ...
        $this->db->select('users.user_id, user_password, user_full_name, user_description,
            user_email, user_is_deleted, user_image, user_input_date, user_last_update, user_majors_id');
        $this->db->select('user_role_role_id, user_roles.role_name');
        $this->db->select('majors.majors_id, majors.majors_name, majors.majors_short_name');
    ...
    function get_role($params = array()) {
        ...
        if($this->session->userdata('uroleid') != '3'){
            $this->db->where('user_roles.role_id != 1 AND user_roles.role_id != 3');
        }
        
        if($this->session->userdata('uroleid') == '3'){
            $this->db->where('user_roles.role_id != 3');
        }
    ...
    function add($data = array()) {
        ...
        if (isset($data['user_majors_id'])) {
            $this->db->set('user_majors_id', $data['user_majors_id']);
        }
