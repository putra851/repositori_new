					...
					<div class="modal-body">
						<div id="p_scents_item">
							<p>
								...
								<input type="text" required="" name="item_name[]" class="form-control" placeholder="Contoh: X (A)">
	...
	<script>
		$(function() {
			...
			$("#addScnt_item").click(function() {
				$('<p><label>Nama Unit POS</label><input required type="text" name="item_name[]" class="form-control" placeholder="Contoh: X (A)"><a href="#" class="btn btn-xs btn-danger remScnt_item">Hapus Baris</a><br><label>Unit Sekolah</label><select required="" name="item_majors_id[]" class="form-control"><option value="">-Pilih Unit Sekolah-</option><?php foreach($majors as $row){?><option value="<?php echo $row['majors_id'] ?>"><?php echo $row['majors_short_name'] ?></option><?php } ?></select></p>').appendTo(scntDiv);
				i++;
				return false;
			});
