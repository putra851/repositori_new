...
<div class="content-wrapper">
				...
				<div class="box">
                	...
                	<div class="row">
                		...
                		<div class="input-group date " data-date="" data-date-format="yyyy-mm-dd">
                			...
                			<input class="form-control" required="" type="text" name="kas_date" id="kas_date" placeholder="Tanggal Kas Masuk" value="<?php echo date('Y-m-d')?>">
                		</div>
                		</div>
                		<div class="col-md-3">
                		</div>
                	...
                	<div class="row">
                		<div class="col-md-3">
                			...
                			<input type="text" required="" name="kas_note" id="kas_note" class="form-control" placeholder="Keterangan Kas Masuk">
                		</div>
                		<div class="col-md-3">
                		</div>
                		<div class="col-md-3">
                			<label>Total *</label>
                			<input type="text" required="" name="kas_value" id="kas_value" class="form-control" placeholder="Total Kas Masuk" readonly="">
                		</div>
                		<div class="col-md-3">
                		</div>
                	</div>
				...
				<div class="box">
                	...
                	<div class="row">
                		...
                		<div id="div_kode">
                			...
                			<select required="" name="debit_account_id" id="debit_account_id" class="form-control" required="">
                		...
                		<div class="col-md-3">
                			<label>Uraian *</label>
                			<input type="text" required="" name="debit_desc" id="debit_desc" class="form-control" placeholder="Uraian Kas Masuk">
                		</div>
                		<div class="col-md-2">
                			<label>Nominal (Rp.)*</label>
                			<input type="text" class="form-control" required="" name="debit_value" id="debit_value" class="form-control" placeholder="Jumlah">
                		</div>
                		<div class="col-md-2">
                		    <label>Pajak</label>
                		    <select name="debit_tax_id" id="debit_tax_id" class="form-control">
	...
	<script type="text/javascript">
		...
		function tampil_data(){
		    ...
		    $.ajax({
		        type  : "POST",
		        url   : "<?php echo base_url()?>manage/debit/data_trx/"+kas_noref,
		        ...
		        success : function(data){
		            ...
		            for(i=0; i<data.length; i++){
		                ...
		                var date = new Date(data[i].debit_temp_date);
                        var dd = date.getDate();
                        ...
                        sum = parseInt(data[i].debit_temp_value);
		                pajak = parseInt(data[i].debit_temp_value*(data[i].debit_temp_tax/100));
		                sumLast = parseInt(sum-pajak);
		                
		                sumAll += parseInt(data[i].debit_temp_value);
		                pajakAll += parseInt(data[i].debit_temp_value*(data[i].debit_temp_tax/100));
		                ...
		                html += '<tr>'+
		                        ...
		                        '<td>'+data[i].debit_temp_desc+'</td>'+
		                        '<td>'+nominal+'</td>'+
		                        '<td>'+data[i].debit_temp_tax+' %</td>'+
		                        '<td>'+data[i].debit_temp_item+'</td>'+
		                        '<td>'+lastNominal+'</td>'+
		                        '<td style="text-align:center;">'+
                                    '<a href="javascript:;" class="btn btn-danger btn-xs item_hapus" data="'+data[i].debit_temp_id+'" title="Hapus"><i class="fa fa-trash"></i></a>'+
                                '</td>'+
		                        '</tr>';
		                        no++
		            }
        ...
        $(document).ready(function(){
            $("#majors_id").change(function(e){
                ...
                $.ajax({ 
                    url: '<?php echo base_url();?>manage/debit/cari_ref',
                    type: 'POST', 
                    data: {
                            'id_majors' : id_majors,
                    },    
        	...
        	$("#majors_id").change(function(e){
                ...
                $.ajax({ 
                    url: '<?php echo base_url();?>manage/debit/cari_kas',
                    type: 'POST', 
                    data: {
                            'id_majors': id_majors,
                    },    
        	...
        	$("#majors_id").change(function(e){
                ...
                $.ajax({ 
                    url: '<?php echo base_url();?>manage/debit/cari_unit_pos',
                    type: 'POST', 
                    data: {
                            'id_majors': id_majors,
                    },    
		...
		$('#btn_selesai').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url   : "<?php echo base_url()?>manage/debit/save_trx",
                //secureuri: false,
        ...
        $('#btn_simpan').on('click',function(){
            ...
            var debit_desc=$("#debit_desc").val();
            var debit_value=$("#debit_value").val();
            var debit_tax_id=$("#debit_tax_id").val();
            var debit_item_id=$("#debit_item_id").val();
            
            if(majors_id != '' && kas_noref != '' && kas_date != '' && kas_account_id != '' && debit_account_id != '' && debit_desc != '' && debit_value != ''){
            $.ajax({
                type : "POST",
                url   : "<?php echo base_url()?>manage/debit/add_trx",
                dataType : "JSON",
                data : {
                        'majors_id':majors_id, 
                        'kas_noref':kas_noref, 
                ...
                success: function(data){
                    if(kas_note == ''){
                        ...
                        var note = kas_note + ', ' + debit_desc;
                    }
                    $('[name="kas_note"]').val(note);
                    $('[name="debit_account_id"]').val("");
                    $('[name="debit_desc"]').val("");
                    $('[name="debit_value"]').val("");
                    $('[name="debit_tax_id"]').val("0");
                    $('[name="debit_item_id"]').val("0");
                    tampil_data();
                },
    			error: function(msg){
    					alert('msg');
        ...
        $('#btn_batal').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url   : "<?php echo base_url()?>manage/debit/cancel_trx",
                dataType : "JSON",
                data : { 
                        'kas_noref':kas_noref,
                    },
                success: function(data){
                    window.location.href = '<?php echo base_url()?>manage/debit';
                },
    			error: function(msg){
    					alert('msg');
    			}
        ...
        $('#btn_hapus').on('click',function(){
            ...
            $.ajax({
            type : "POST",
            url  : "<?php echo base_url('manage/debit/del_trx')?>",
            dataType : "JSON",
                    data : {
                            'id': id
                        },
