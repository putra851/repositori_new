...
class Izin_set extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model(array('student/Student_model', 'izin/Izin_model', 'period/Period_model', 'setting/Setting_model', 'logs/Logs_model'));
...
// Santri
    ...
    $data['izin']         = $this->Izin_model->get(array('period_id'=>$periodID, 'student_id'=>$siswa['student_id'], 'order_by' => 'izin_id'));
    $data['izinSum']      = $this->Izin_model->get_sum(array('period_id'=>$periodID, 'student_id'=>$siswa['student_id']));

    $data['majors']     = $this->Student_model->get_majors();

    $data['title']      = 'Izin Santri';
    $data['main']       = 'izin/izin_list';
    ...
    public function add(){
		if ($_POST == TRUE) {
			...
			$period = $this->input->post('izin_period_id');
			$nis    = $this->input->post('izin_student_nis');
			
			$start = $this->input->post('izin_time_start');
			$end   = $this->input->post('izin_time_end');
			
			$izin_time;
		    
			$params['izin_date']       = $this->input->post('izin_date');
			$params['izin_time']       = $start . ' - ' . $end;
			$params['izin_student_id'] = $this->input->post('izin_student_id');
			$params['izin_period_id']  = $this->input->post('izin_period_id');
			$params['izin_user_id']    = $this->session->userdata('uid');

			$this->Izin_model->add($params);
		}
		
		$this->session->set_flashdata('success',' Tambah Izin Berhasil');
		redirect('manage/izin?n='.$period.'&r='.$nis);
	}
	...
	public function delete($id = NULL) {
		if ($_POST) {
			...
			$this->Izin_model->delete($id);
			    $this->session->set_flashdata('success', 'Hapus Izin Berhasil');
			
			redirect('manage/izin?n='.$period.'&r='.$nis);
		} elseif (!$_POST) {
		        $this->session->set_flashdata('failed', 'Hapus Izin Gagal');
...
// Santri
    ...
    $data['period']     = $this->Period_model->get($params);
    $data['izin']       = $this->Izin_model->get(array('period_id'=>$periodID, 'student_id'=>$siswaID));
    $data['izinSum']    = $this->Izin_model->get_sum(array('period_id'=>$periodID, 'student_id'=>$siswaID));
    ...
    $html = $this->load->view('izin/izin_cetak_pdf', $data, TRUE);
    $data = pdf_create($html, 'Buku_Izin_'.$siswa['student_full_name'].'_'.date('Y-m-d'), TRUE, 'A4', TRUE);
  }
  
  private function barcode2($sparepart_code, $barcode_type=39, $scale=6, $fontsize=1, $thickness=30,$dpi=72) {

  $this->load->library('upload');
  $config['upload_path'] = FCPATH . 'media/barcode_izin/';
  ...
  try {
    ...
    $drawing->setFilename( FCPATH .'media/barcode_izin/'. $sparepart_code.'.png');
  ...
  public function report(){
      $q = $this->input->get(NULL, TRUE);
		...
		$data['izin']       = $this->Izin_model->get_sum($params);
        
		$config['suffix'] = '?' . http_build_query($_GET, '', "&");
        
        $data['title'] = 'Rekap Izin Santri';
        $data['main'] = 'izin/izin_report';
        $this->load->view('manage/layout', $data);
  }
  
  public function izin_excel() {
        $q = $this->input->get(NULL, TRUE);
        ...
        $this->load->view('izin/izin_xls', $data);
    }
    ...
    $data['izin']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, madin_name, izin_id, izin_date, izin_time FROM izin JOIN student ON student.student_id = izin.izin_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id JOIN madin ON madin.madin_id = student.student_madin WHERE izin_id = '$id'")->row_array();
    ...
    $html = $this->load->view('izin/izin_pdf', $data, TRUE);
    $data = pdf_create_thermal($html, 'Izin_Keluar_'.$siswa['student_full_name'].'_'.date('Y-m-d'), TRUE, 'A4', TRUE);
  }
